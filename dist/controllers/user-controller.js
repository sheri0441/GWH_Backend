...
const userAndCartHandler = async (req, res, next) => {
    ...
    let validAuthId;
    try {
        validAuthId = (0, valibot_1.parse)(auth_1.authIdSchema, authId);
    }
    catch (error) {
        next(new http_error_1.HttpError("AuthId required", 422));
        return;
    }
    let validUser;
    try {
        validUser = (0, valibot_1.parse)(user_1.userSchema, body);
    }
    catch (error) {
        next(new http_error_1.HttpError("There is some error in your user  data", 422));
        return;
    }
...
const updateUserCart = async (req, res, next) => {
    ...
    try {
        ...
        next(new http_error_1.HttpError("There is some validation error related to email.", 422));
        return;
    }
    let validCartList;
    try {
        validCartList = (0, valibot_1.parse)(cart_1.cartSchema, cart);
    }
    catch (error) {
        next(new http_error_1.HttpError("AuthId required", 422));
        return;
    ...
    if (!user) {
        next(new http_error_1.HttpError("You are not registered on our website.", 404));
        return;
    }
    let updatedUser;
    try {
...
const deleteUser = async (req, res, next) => {
    ...
    try {
        ...
        next(new http_error_1.HttpError("There is some error in the database.", 500));
        return;
    }
    try {
        await prisma_1.prisma.deleteUser.create({
            data: {
                auth: validAuthId,
            },
        });
